simulate_4a_mix <- function(params_inc, params_inf, params_iso,
  ...
  valid_si <- valid_si[sample(nrow(valid_si), nsim_local2), ]
  
  invalid_si <- data.frame(
    si = rbeta(nsim_local2, shape1 = alpha_invalid, shape2 = beta_invalid),
    nu = rgamma(nsim_local2, shape = params_iso$shape, scale = params_iso$scale)
  )
  ...
  for (idx in seq_len(nsim_local2)) {
    toss <- runif(1)
    if (toss <= pinvalid) {
      simulated_si <- rbind(
  ...
  simulated_si$toss2 <- runif(nsim_local2, min = 0, max = 1)
  
  simulated_si_recalled <- simulated_si%>%
    dplyr::mutate(recall = exp(-abs(si-nu)*p_recall))%>%
    dplyr::filter(toss2 < recall)
  
  simulated_si_recalled <- simulated_si_recalled[sample(nrow(simulated_si_recalled), nsim),]
  
  list(
    valid_si = valid_si,
    invalid_si = invalid_si,
    simulated_si = simulated_si[,c("si", "nu", "group")],
    simulated_si_recalled = simulated_si_recalled[,c("si", "nu", "group")]
  )
}
