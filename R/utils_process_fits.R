...
#functions
      ...
      table2_fun <- function(samples){
        ...
        tab2 <- data.frame(best_pars = TOST_summary_func(samples$TOST_bestpars),
                           ...
                           CrI_2.5 = CrI_2.5,
                           CrI_97.5 = CrI_97.5)
        mean_si <- c(mean(samples$SI_bestpars$SI), mean(samples$SI_meanpars$SI), 
                     quantile(colMeans(samples$SI_post), probs = c(0.025, 0.975)))
        median_si <- c(median(samples$SI_bestpars$SI), median(samples$SI_meanpars$SI), 
                       quantile(colMedians(as.matrix(samples$SI_post)), probs = c(0.025, 0.975)))
        sd_si <- c(sd(samples$SI_bestpars$SI), sd(samples$SI_meanpars$SI), 
