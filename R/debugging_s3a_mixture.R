...
for(i in 1:N){
  out_invalid[i] <- invalid_lpdf(
    x = SI[i], max_si = max(SI) + 0.001, min_si = min(SI) - 0.001,
    alpha_invalid = 0.5, beta_invalid = 0.5)
}
...
for (i in 1:N) {
  if(SI[i] > -offset){
    target_width[i] <- log(p_inv) + out_invalid[i] +
      log(1 - p_inv) + out_width[i]
  } else{
    target_width[i] <- out_invalid[i]
  }
}
...
for (i in 1:N) {
  if(SI[i]> -offset){
    target_nowidth[i] <- log(p_inv) + out_invalid[i] +
      log(1 - p_inv) + out_nowidth[i]
  } else {
    target_nowidth[i] <- out_invalid[i]
  }
}
...
mat <- data.frame(target_width, target_nowidth)
...
ggplot() +
  geom_line(aes(SI, target_nowidth), col = "red") +
  geom_line(aes(SI, target_width), col = "blue")
