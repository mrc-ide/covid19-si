...
sim_data <- replicate(
  n_datasets, better_simulate_si(
    params_inc, params_inf, params_iso, param_offset, max_shed, 4e4
  )
)
...
for(i in 1:n_datasets){ # run multiple simulations to check variability
  ...
  p[[i]] <- ggplot() +
    ...
    geom_density(
      aes(filtered_500[[i]]$t_1), fill = "red", col = NA, alpha = 0.2
    ) +
    geom_density(
      aes(filtered$t_1), fill = "red", col = NA, alpha = 0.2
    ) +
    geom_vline(xintercept = param_offset)+
    theme_minimal()
...
test_fit <- ggmcmc(ggs(fit[[2]]), here::here("figures/test2.pdf"))
...
saveRDS(fitted_params, file = "fitted_params_check.rds") 
...
ggsave("figures/recovered_inf.png", q)
