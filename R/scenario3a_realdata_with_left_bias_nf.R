...
## Treat NAs as local cases
data$infector_returned_fromOtherCity[is.na(data$infector_returned_fromOtherCity)] <-
  "local"
## Set return date of local/NA cases to way back in the past
local <- which(data$infector_returned_fromOtherCity == "local")
data$infector_returnDate_fromOtherCity[local] <-
   data$infector_onsetDate[local] + 20


data$delay_import <-
  data$infector_onsetDate - data$infector_returnDate_fromOtherCity


real_data <- data %>%
  mutate(si = as.numeric(si))%>%
  rename(nu = onset_first_iso, rho = delay_import) %>%
  filter(!is.na(nu))
...
fit_3a_real <- stan(
  file = here::here("stan-models/scenario3a_mixture_left_bias_nf.stan"),
  data = list(
    N = length(real_data$si),
    si = real_data$si,
    rho = as.numeric(real_data$rho),
    max_shed = max_shed,
